...
class navigationControl(Node):
    ...
    def listener_callback(self, msg):
        ...
        # Print listener_comeback data
        print(f"Listener Data: x={self.x}, y={self.y}, yaw={self.yaw}")

        # Map the base_link data to the costmap grid
        if self.map_data:
            grid_x, grid_y = self.world_to_grid(self.x, self.y)
            print(f"Mapped to grid: x={grid_x}, y={grid_y}")
            self.update_robot_position_in_grid(grid_x, grid_y)
    ...
    def write_costmap_to_file(self, msg):
        ...
        #             index = i * width + j
        #             if msg.data[index] == -1:
        #                 grid[height - i - 1, j] = 2  # Flip the grid vertically
        #             elif msg.data[index] > 0:
        #                 grid[height - i - 1, j] = 1  # Flip the grid vertically
        #             else:
        #                 grid[height - i - 1, j] = 0  # Flip the grid vertically
    ...
    def update_robot_position_in_grid(self, grid_x, grid_y):
        ...
        print(f"Robot position updated to grid coordinates: ({grid_x}, {grid_y})")
