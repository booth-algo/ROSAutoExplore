...
def save_map():
    ...
    try:
        # Command to run
        command = ["ros2", "run", "nav2_map_server", "map_saver_cli", "-f", "~/uploader/."]
    ...
    try:
        command = ["python3", "/home/kevin/uploader/upload.py"]
        result = subprocess.run(command, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
class navigationControl(Node):
    ...
    def exp(self):
        ...
        while True: #Wait until sensor data arrives.
            if not hasattr(self,'map_data') or not hasattr(self,'listener_data') or not hasattr(self,'scan_data'):
                time.sleep(0.1)
                continue
            if self.kesif == True:
    ...
    def listener_callback(self,msg):
        self.listener_data = msg
        self.x = msg.transform.translation.x
        self.y = msg.transform.translation.y
        self.yaw = euler_from_quaternion(msg.transform.rotation.x,msg.transform.rotation.y,
