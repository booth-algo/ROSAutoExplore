...
#             if matrix[i][j] == 0.0:  # Check if the cell is free space
#                 # Check 8-connected neighbors for unknown or partially known space
#                 if (i > 0 and matrix[i-1][j] < 0) or (i < height-1 and matrix[i+1][j] < 0) or \
#                    (j > 0 and matrix[i][j-1] < 0) or (j < width-1 and matrix[i][j+1] < 0) or \
#                    (i > 0 and j > 0 and matrix[i-1][j-1] < 0) or (i > 0 and j < width-1 and matrix[i-1][j+1] < 0) or \
#                    (i < height-1 and j > 0 and matrix[i+1][j-1] < 0) or (i < height-1 and j < width-1 and matrix[i+1][j+1] < 0):
#                     matrix[i][j] = 2
#             elif matrix[i][j] < 0:  # Check if the cell is unknown
#                 # Check 8-connected neighbors for free space to mark as potential frontiers
#                 if (i > 0 and matrix[i-1][j] == 0) or (i < height-1 and matrix[i+1][j] == 0) or \
#                    (j > 0 and matrix[i][j-1] == 0) or (j < width-1 and matrix[i][j+1] == 0) or \
#                    (i > 0 and j > 0 and matrix[i-1][j-1] == 0) or (i > 0 and j < width-1 and matrix[i-1][j+1] == 0) or \
#                    (i < height-1 and j > 0 and matrix[i+1][j-1] == 0) or (i < height-1 and j < width-1 and matrix[i+1][j+1] == 0):
...
def fGroups(groups):
    ...
    top_five_groups = [g for g in sorted_groups[:5] if len(g[1]) > threshold]    
...
def exploration(data,width,height,resolution,column,row,originX,originY):
        ...
        # Check if there are groups after assign_groups call
        print("groups:", groups)
...
class navigationControl(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(OccupancyGrid,'cmap',self.map_callback,10)
